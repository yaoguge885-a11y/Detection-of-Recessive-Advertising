...
- 记录数：3,312
- 当前结论：阶段3已正式通过。A补件的13项SHA全部通过；3,312条canonical完整分区为2,932批准与380排除，交集0、遗漏0；正式候选、审批和allowlist一致，Schema为2,932/2,932，隐私复扫public/interim/raw为2,932/377/3。正式产物已落入 `data/reports/m1/privacy/formal_3312_v2/`。使用范围仅限受控内部M1标注、κ和Gold实验，外部数据集与媒体发布仍受阻。阶段4的A/B固定20条结构验收也已通过，当前只等待双方共同确认 `qwen3.5:4b` 或 `--no-llm`，之后才能正式锁定阶段5四批。
...
## B 阶段 3 历史结论（截至 2026-08-10 早期状态，已被下节更新覆盖）

- B 主责的初始隐私复核、媒体审核、正式审批 JSON、条款抽查签字、可机械执行的文本脱敏候选、补充队列 AI 首轮分流及 581 条远程视频替换队列的 AI 视觉初筛均已完成。
- 本节原记录的 1,696 条补充人工决定已由后续保守排除路线替代，不再执行；当前状态以文首和下方“2026-08-21最终验收更新”为准。

## 2026-08-14 复核更新

- B 已完成新的 308 条 spotcheck；它不是既往 S-001～S-308。验收报告 `privacy_spotcheck_10pct_validation_B.json` 为 `passed=true`，0 错误、0 警告。
- 2,992/320 集合表面覆盖3,312条且无交集，但当时有两条B已判为redact的记录仍在批准集内，因此必须更正并重新对账；具体ID只保留在本地受限审计文件中。
- 复核包审批绑定 SHA-256 `b667eb...`，当前 canonical 为 `e9ccad...`；A 必须提供匹配的数据/生成链或重新绑定并复扫，不能直接提升当前文件。
- 7 条既有媒体风险均已在排除集；A 的 30 条边界抽查已完成。
- spotcheck 与集合修正提案已完成；阶段 3 正式化、阶段 5 正式锁批与 Round 1 仍必须阻断，直到 A 确认 2,932/380 修正版、解决数据 SHA/1,095 条 terms_checked 问题并完成 A 侧证据。

## 历史待办（已由2026-08-18补件关闭）

1. A已通过回执绑定实际采集/条款证据与受控内部实验范围；外部发布仍禁止。
2. A已完成B高风险/边界判断抽查。
3. 3,312条记录均已有可核验的 `provenance.terms_checked_at`，生成链与canonical SHA一致。

## 历史事实门（已关闭）

在早期数据与条款状态下曾只有1,974条能进入public，低于2,050条门槛。该阻断已通过匹配SHA的terms-checked canonical、A/B集合对账、正式Schema和隐私复扫关闭；当前正式public为2,932条。

## 2026-08-17 阶段 5 前最终 B 状态

- B 在阶段 5 前需要亲自完成的人工任务均已完成：541/541 条隐私判断、308/308 条 spotcheck、B 的固定 20/20 条 Qwen 校准复核。
- A 已由 B 转述为口头确认阶段 3 交付包，但仓库尚未收到 A 的书面回执；口头确认不替代正式数据链、条款证据、SHA 绑定和最终复扫。
- 已生成 A 剩余门禁交付包 `M1_stage4_A_pending_gates_handoff_20260817.zip`，ZIP SHA-256 为 `D56F4B42FEDED8514F9D7019C4CEBCD5DC4D3D9413BDEE2C901D7E2E49345089`。包内包含阶段 3 回执模板、A 独立 20 条页面、固定 manifest 和 362 个媒体，不包含 B 的人工校准答案。
- A 返回书面回执与 `qwen_calibration_20_human_review_A.json` 后，B 执行机械验收和 A/B 汇总；最终模型路线必须由 A/B 共同确认。
- 阶段 5 工具与 Round 1 预检工具已经准备，但正式锁批仍需阶段 3/4 总门通过。当前可审计复扫为 public 1,837、interim 1,095，且正式 `formal_eligible_candidates.jsonl` 不存在，因此不得提前开始 Round 1 正式盲标。

## 2026-08-21最终验收更新

- A补件ZIP按包内 `SHA256SUMS` 验证为13/13通过，无漏列文件和不安全路径。
- 阶段3正式池为2,932批准/380排除，完整覆盖3,312条canonical；正式候选、审批和allowlist集合与顺序均通过机械核对。
- A修正后的固定20条与B既有20条联合结构验收为 `passed=true`；机械工具不替A/B选择最终模型路线。
- 阶段5已完成 `preflight_not_formal` 演练：100、150、180、1,620四批共2,050个唯一ID，批间交集0，manifest与导出JSONL数量和顺序一致。
- 旧 `formal_3312_v1` Round 1已归档，不再使用。其失效依据是绑定旧候选池且包含已退出当前正式池的记录；不能通过人工替换单条继续沿用。
- 当前唯一未决门是A/B共同确认Gold assisted使用 `qwen3.5:4b` 或 `--no-llm`。该决定落盘前，不得生成正式 `locked_batches/` 或启动Round 1。
