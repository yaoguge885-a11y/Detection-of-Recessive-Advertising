#!/usr/bin/env python3
"""Generate B's local interactive review page for the 20-item Qwen calibration sample."""
...
def render(items: list[dict[str, Any]], stats: dict[str, int], fingerprint: str) -> str:
...
<title>M1 Qwen 20 条合理性复核 — B</title>
<style>
...
<header><h1>M1 Qwen 20 条合理性复核 — B</h1><div class="bar"><span id="progress"></span><select id="filter"><option value="all">全部</option><option value="pending">未完成</option><option value="done">已完成</option></select><button id="prev">← 上一条</button><span id="counter"></span><button id="next">下一条 →</button><button id="export">校验并导出 JSON</button></div></header>
<main><div class="guide"><b>标签顺序：</b>先判断是否在范围内，再判断是否有充分商业推广意图，最后看是否有明确广告/赞助/合作/赠送/受邀/平台商业标识。明广=商业意图充分且有明确披露；暗广=商业意图充分、披露区域完整但无明确披露；非广=商业意图证据不足。建议先独立选 B 标签，再展开 Qwen 建议。</div><div id="view"></div></main>
<div id="lightbox"><button id="close">×</button><img id="large" alt="放大预览"></div>
<script>
const items={data},stats={stats_json},fingerprint={fingerprint_json},key='m1-qwen-calibration-20-B-v1';let saved=JSON.parse(localStorage.getItem(key)||'{{}}'),filtered=[],current=0;
...
<div class="review"><h3>B 独立复核</h3><div class="field"><b>1. B 人工标签：</b>${{radios('label',[['明广','明广'],['暗广','暗广'],['非广','非广'],['uncertain','uncertain'],['out_of_scope','out_of_scope']],v.label)}}</div>
...
def main() -> int:
    ...
    output = (args.output or repo / "data" / "reports" / "m1" / "qwen_calibration_20_review_B.html").resolve()
    ...
    output.write_text(render(items, stats, str(manifest["dataset_fingerprint_sha256"])), encoding="utf-8")
