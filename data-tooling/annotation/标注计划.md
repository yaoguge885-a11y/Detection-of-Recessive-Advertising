文件创建工具被禁用了，我直接把完整的 Markdown 文档输出在下面，你可以复制保存为 `docs/co-pilot-auto-judge-design.md`：
...
帖子加载
  ...
  │
  └─► Qwen3 8B via Ollama (本地, ~5-15s) ──► 综合判定
         │
...
Qwen3 8B 是纯文本模型，不支持视觉输入。图片分析继续由 YOLO+OCR（`auto_image_annotate.py`）完成，产出结构化的 `detected_elements` 和 `visual_evidence_codes`，作为文本证据传入 Qwen3 8B。

**这比直接把图扔给 8B 多模态模型更可靠**，因为 YOLO+OCR 的确定性输出（"图片3含品牌Logo + 价格数字"）消除了视觉幻觉风险。
...
| `_llm_suggestion.model` | `"qwen3:8b"` | 使用的模型 |
...
# 2. 验证模型可用
ollama run qwen3:8b "你好，请用JSON格式输出：{\"status\": \"ok\"}"
```
...
| 风险 | 概率 | 缓解措施 |
|------|------|---------|
| Qwen3 8B 判断质量不足 | 中 | 从 0.85 保守阈值起步；人工复核 30 条验证 |
...
1. **多模型投票**：同时跑 Qwen3 8B + DeepSeek，一致时置信度 +0.1
