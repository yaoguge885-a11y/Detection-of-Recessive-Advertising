...
> 当前实测基线：2026-08-09，分支 `P3-合并baseline`；各专项历史快照的日期与数字保持原样。
> P1 合并基线：P1 提交前缀 `43c59ac`，合并提交前缀 `ca8fc2d`
...
| 功能 | 当前状态 | 本手册中的验证方式 |
| --- | --- | --- |
| P1 Schema v1.0/v1.2 与提交资产 | 已合并；v1.0保留30条合成提交资产，P3运行时严格支持v1.1/v1.2扩展Schema | 两套 P1 资产校验器；`tests\data` |
...
| 独立合并历史分类基线 | `baseline/`四方法固定Logistic Regression、M1/split fail-closed、共同cohort、版本/哈希和隐私安全聚合报告；最新专项`54 passed` | 第4.17节；仅合成fixture工程验证，正式Gold=0且M1未通过 |
...
| P5.2开发者研究工作台 | 同源无构建`/workbench`已实现：单条/批量、能力门控URL、完整run九区、复制/UTF-8下载、键盘和响应式布局；默认URL适配器为空 | `tests\web`、`tests\test_app.py`、`tests\api`；第4.12节 |
| P5.3/P5.4 synthetic fixture工程 | 小红书normal/video与B站video/opus/article五个结构仿真案例通过确定性解析，统一PostRecord/CaptureStatus，四目标模态与披露人工修正有回归；默认不注册/不请求真实平台 | `tests\adapters\platforms`、`tests\api`、`tests\web`；第4.11.2节 |
...
- A2A 专家间任务交换；
- P5.3/P5.4 synthetic fixture 只覆盖五个结构仿真案例和离线适配器契约；不能证明真实小红书/B站页面兼容、真实平台请求、来源条款/隐私/安全审批、P5.5/P5.6、M5、M1/M4、研究有效性或浏览器插件。P5.2工作台只完成工程门，不代表四人团队UAT或研究验收。
...
2026-08-09 P5.7收口后的新鲜实测快照：

```text
495 passed, 2 skipped, 1 warning
```

正确理解：

- `495 passed`：当前收集到的默认测试全部通过；
- `2 skipped`：两个真实 YOLO/EasyOCR 测试被有意跳过；
- `1 warning`：既有Starlette/httpx TestClient弃用提示；
- 这两个 skip 是默认策略，不是缺陷；
...
- MCP、Chroma、CreatorShift图节点、P4夹具基准、P5.1批量/URL服务边界、P5.2同源工作台和选择性评估等已实现模块可以在当前虚拟环境中导入和测试。
...
- 不能证明 A2A或浏览器插件已经存在，也不能把P5.2工作台工程测试当作团队UAT、真实平台采集或M5；
...
这是P5.1在2026-07-30的历史准入说明；其后P5.7已补齐DNS、重定向、Cookie/Token脱敏等离线安全门。它仍不能证明真实小红书/B站页面可访问、登录/验证码/反爬绕过可用或进程内preview具备跨进程语义。P5.3～P5.6、团队UAT和M5仍未完成。
...
2026-08-09新鲜实测：`144 passed in 5.31s`。最终全量为`495 passed, 2 skipped, 1 warning in 12.71s`；既有warning仍是Starlette/httpx弃用提示。

生成物扫描必须传入明确目录，不默认扫描整个仓库：

```powershell
& '.\.venv\Scripts\python.exe' scripts\security\scan_p5_7_artifacts.py `
  --path <generated-artifact-directory>
```

干净验收run实测输出`[]`、退出码0；不安全合成fixture退出码1，只输出规则、行号、长度和SHA-256，不输出匹配文本。

这组测试覆盖重定向逐跳复核、DNS变化/私网阻断和IP固定、URL组件脱敏、网页提示注入隔离、平台正文不能扩大工具授权、路径穿越/异常媒体引用、MCP/假A2A超时与伪造/越权、日志/报告/run泄漏扫描。全程未请求真实平台URL。它不完成P5.3/P5.4真实适配器、P5.5真实A2A、P5.6对照、团队UAT、M1、M4或M5。

### 4.11.2 P5.3/P5.4 synthetic fixture工程验收（2026-08-09）

本节只记录五个结构仿真案例：小红书 `normal`、`video`，以及 B站 `video`、`opus`、`article`。每个案例固定包含 `source.html`、`source_state.json`、`manifest.json`、`expected_post.json`；解析器零网络、两次解析输出字节等价，并统一生成 `PostRecord/CaptureStatus`。四目标模态为 `text`、`image`、`comment`、`disclosure`；披露人工修正有确认run和 `user_corrections` 审计回归。manifest 明确声明 synthetic、无真实用户数据、无需网络、未验证真实平台兼容且未完成条款审批；默认应用不注册平台适配器。

聚焦 Python 回归（在`implicit-ad-agent`目录）：
...
这组证据只证明 synthetic fixture 工程契约、离线解析、统一输出、披露修正和回归测试；不证明真实页面兼容、真实平台请求、来源条款/隐私/安全审批、P5.5/P5.6、M5、M1/M4、研究有效性或浏览器插件。它也不替代四人真人UAT（当前仍为0/4）。

### 4.12 P5.2开发者研究工作台工程门

聚焦回归（在`implicit-ad-agent`目录）：

```powershell
python -m pytest tests\web tests\test_app.py tests\api -q
```

2026-07-31实测：`28 passed, 1 warning in 3.18s`。再运行`python -m pytest -q`，本轮全量为`390 passed, 2 skipped, 1 warning in 12.13s`；warning是既有Starlette/httpx TestClient弃用提示。
...
在单条页输入`品牌合作，广告，限时购买`，勾选“已完整采集披露面”，点击“开始分析”。预期页面显示完成状态和完整持久化run；结论、覆盖/缺失、证据、CreatorShift、历史、法规、轨迹、可读报告和raw JSON均可见。浏览器复制按钮应写入与页面一致的Markdown或可解析JSON；下载仅在显式点击后生成UTF-8文件。
...
```json
{
  "items": [
    ...
    {"post_id": "missing-text"},
    {"text": "今天分享普通日常", "capture_complete": true}
  ]
}
```

预期为`2成功 / 1失败 / 3总计`；失败项显示稳定错误而不阻断两条成功run，两个“查看”按钮可分别加载各自run。默认`/api/v1/capabilities`没有平台注册项，因此URL页必须显示未配置说明，URL输入和预览按钮保持禁用；能力请求未完成或失败时也不得发起预览请求。
安全与远程资产扫描（从仓库根目录运行）：

```powershell
$script = Get-Content -Raw -Encoding UTF8 implicit-ad-agent\impad\web\workbench.js
$html = Get-Content -Raw -Encoding UTF8 implicit-ad-agent\impad\web\index.html
$css = Get-Content -Raw -Encoding UTF8 implicit-ad-agent\impad\web\workbench.css
$forbidden = @('innerHTML', 'outerHTML', 'insertAdjacentHTML', 'localStorage', 'sessionStorage', 'indexedDB', 'document.cookie', 'serviceWorker')
$forbidden | ForEach-Object { if ($script.Contains($_)) { throw ('forbidden workbench API: ' + $_) } }
if ($html -match 'https?://' -or $css -match 'url\(https?://') { throw 'remote workbench asset reference found' }
$secretPattern = 'api[_-]?key\s*[=:]\s*[^\s|]+|access[_-]?token\s*[=:]\s*[^\s|]+|bearer\s+[A-Za-z0-9][^\s|]*'
$secretMatches = @(rg -n --no-heading -o $secretPattern implicit-ad-agent\impad\web HANDOFF.md docs | ForEach-Object { $_.TrimEnd([char]96) })
...
$allowedLinePattern = '^{0}:\d+:{1}$' -f [regex]::Escape($allowedHistoricalPath), [regex]::Escape($allowedHistoricalLiteral)
$expectedHistoricalMatches = @($secretMatches | Where-Object { $_ -match $allowedLinePattern })
$unexpectedSecretMatches = @($secretMatches | Where-Object { $_ -notmatch $allowedLinePattern })
if ($expectedHistoricalMatches.Count -ne 2 -or $unexpectedSecretMatches.Count -ne 0) { $secretMatches; throw 'secret scan expected exactly 2 historical fixture matches and 0 unexpected matches' }
Write-Output 'secret scan: 2 expected historical fixture matches, 0 unexpected matches'
```

这不是手工排除：命令只允许指定P5.1历史计划路径中的两处指定fixture。若路径、值或数量变化，或任意其他匹配出现，命令会失败。

浏览器验收清单：确认单条、混合批量与结果切换；URL默认禁用；ArrowLeft/ArrowRight/Home/End切换tab且焦点/选中状态一致；复制和实际UTF-8下载；1440px与390px无横向溢出。Task 7真实浏览器记录为单条`run_bf717e85fd934afc81d227edf3720ab6`、批量2/1/3、GREEN新增console errors=`0`；临时截图为`C:\Users\31729\AppData\Local\Temp\impad-p5-workbench\workbench-desktop.png`与`workbench-narrow.png`，不提交。

这组测试可证明同源、无构建客户端正确使用既有本地API并安全渲染当前run；不能证明四人团队UAT、真实小红书/B站采集、A2A、P5.3～P5.7、M1/M4/M5、分类精度、CreatorShift增益或法规完备性。默认P1资产校验器的`VALIDATION PASSED`也不等于M1完成。

### 4.13 一次运行 P1 数据层与本轮独立模块
...
### 4.15 分置信度自动判断标注系统（co-pilot-auto-judge）

> 设计文档：`docs/co-pilot-auto-judge-design.md`
> 核心模块：`data-tooling\annotation\auto_judge.py`
> 配套工具：`batch_pre_annotate.py`（批量预标注）、`flet_annotator.py`（GUI）、`manual_review_annotate.py`（CLI）
> 本地模型：Ollama + `qwen3.5:9b`（`ollama pull qwen3.5:9b`）

单元测试（不依赖 Ollama，mock 掉推理调用）：

```powershell
python -m pytest data-tooling\annotation\tests\test_auto_judge.py -q
```
...
- 三级置信度分类（≥0.85 自动 / 0.55–0.84 建议 / <0.55 纯人工）及边界值；
- 标签归一化（中文 ↔ 代码 ↔ 非法值回退）；
- 6 维关键词向量与导购压力分；
- 关键词失败回退规则（明广 0.90 自动保存 / 暗广 0.45 强制人工 / 无信号不做建议）；
- 自动保存记录结构（`annotation_method: "auto_accepted"`、`_llm_suggestion`）；
- Ollama 失败时完整管线降级与三级分流。

自检脚本（不联网，验证关键词与 Ollama 探测）：

```powershell
python data-tooling\annotation\auto_judge.py
```

批量预标注（真实 Ollama 推理，验证模型可用性）：

```powershell
ollama list   # 应包含 qwen3.5:9b
...
python data-tooling\annotation\manual_review_annotate.py `
  ...
  --ollama-backend `
  --ollama-model qwen3.5:9b `
  --limit 20 --no-supplement
```

GUI 工作台（自动模式 + 阈值滑块 + 底部状态栏）：
...
- 自动保存记录 `annotator_id="system"`、`annotation_method="auto_accepted"`，**不参与双人标注 κ 计算**；
- 中置信度记录 `annotation_method="human"`，由人工确认后保存；
- 自动标注抽样 30 条人工复核，一致率 < 95% 时上调阈值至 0.90。

### 4.16 P1→P3 Schema v1.2 与本地 M1 数据复核

以下命令只读取解压后的本地数据，并把聚合报告写入 `data\reports\m1`。真实正文、媒体、URL 和 ID 映射不得暂存或提交。`data\run_outputs` 已被 Git 忽略。
...
2026-07-31 对用户提供 ZIP 的实测结果：`TOTAL 2901 / VALID 2901 / INVALID 0`。未加 `--privacy-scan` 时，结果只证明结构有效，不证明隐私审批通过。
...
关键结果：2,901 个唯一候选、108 个创作者、0 个重复帖子；Bilibili 2,182 / WeChat 719；14,174 个唯一媒体引用全部可定位。数据集指纹为 `adb39f1840df62cbeef52faabde85177536478c1c06d37bbb747a9a2bb59a3a5`。
...
当前预期退出码为 `2`，报告为 `passed=false`。这是正确的保守结果：候选差99、Gold为0、正式第二轮κ待复核、条款与隐私审批未完成、无泄漏切分缺失、Dataset Card未审批。
...
.\.venv\Scripts\python.exe -m pytest `
  tests\adapters\test_p1_schema_adapter.py `
  tests\test_agents.py -q
```

应覆盖v1.0、v1.1兼容、v1.2、未知版本拒绝和v1.2未知字段拒绝。通过这些测试不等于M1通过。
...
Set-Location implicit-ad-agent
.\.venv\Scripts\python.exe -m pytest `
  tests\data\test_data_governance_tools.py `
  tests\data\test_merge_incremental.py -q
```

2026-07-31实测：`30 passed`。其中明确验证`auto_accepted`、`system`（含首尾空白变体）、空白/缺失标注者ID、缺失人工方法和归一化后同一标注者不能进入κ或Gold；增量合并的v1.1→v1.2输出必须先通过权威Schema，非法记录不得写入目标。

### 4.17 合并历史分类基线（独立工程包）

以下命令均从已打开的仓库根目录运行，并显式使用项目虚拟环境解释器；只读合成夹具，不写入真实私有数据路径。依赖安装可能访问包索引，其余命令为零Key、零联网工程检查。
...
最新实测：baseline专项 `54 passed`（contracts22/features14/runner13/CLI5），`compileall`退出码为0。该结果只证明独立基线工程行为，不是M1或M4验收。

运行四种方法的合成端到端链：

```powershell
& '.\implicit-ad-agent\.venv\Scripts\python.exe' -m baseline.cli synthetic `
  --content baseline\tests\fixtures\synthetic_content.jsonl `
  --gold baseline\tests\fixtures\synthetic_gold.jsonl `
  --train-ids baseline\tests\fixtures\train_ids.txt `
  --dev-ids baseline\tests\fixtures\dev_ids.txt `
  --test-ids baseline\tests\fixtures\test_ids.txt `
  --split-report baseline\tests\fixtures\synthetic_split_report.json `
  --m1-gate baseline\tests\fixtures\synthetic_gate.json `
  --fixture-metadata baseline\tests\fixtures\fixture_metadata.json `
  --output baseline\synthetic_report.json
```

用 `ConvertFrom-Json` 检查报告模式、四个方法、研究声明和隐私边界；检查完成后只删除本次生成的聚合报告：

```powershell
$reportPath = 'baseline\synthetic_report.json'
$report = Get-Content -Raw -Encoding UTF8 -LiteralPath $reportPath | ConvertFrom-Json
$expectedMethods = @('single_post', 'history_mean', 'history_max', 'history_ema')
$actualMethods = @($report.methods.psobject.Properties.Name)
if ($report.mode -ne 'synthetic' -or `
    $report.dataset_kind -ne 'synthetic_fixture' -or `
    $report.research_claims_allowed -ne $false) {
  throw 'synthetic report mode or research boundary is wrong'
}
if (($actualMethods -join ',') -ne ($expectedMethods -join ',')) {
  throw 'synthetic report methods are wrong'
}
$leaks = @(Select-String -LiteralPath $reportPath `
  -Pattern @('新品推荐，优惠价格', 'fixture_creator_') -SimpleMatch)
if ($leaks.Count -ne 0) {
  throw 'synthetic report contains fixture text or creator identifiers'
}
Remove-Item -LiteralPath $reportPath -Force
```

当前正式门禁仍应在训练前拒绝。使用仓库现有聚合 `m1_gate_report.json` 和合成输入仅验证 fail-closed 行为，不把合成夹具当作正式数据：

```powershell
$formalOutputPath = Join-Path ([System.IO.Path]::GetTempPath()) `
  ('baseline-formal-' + [guid]::NewGuid().ToString('N') + '.json')
$formalOutput = & '.\implicit-ad-agent\.venv\Scripts\python.exe' -m baseline.cli formal `
  --content baseline\tests\fixtures\synthetic_content.jsonl `
  --gold baseline\tests\fixtures\synthetic_gold.jsonl `
  --train-ids baseline\tests\fixtures\train_ids.txt `
  --dev-ids baseline\tests\fixtures\dev_ids.txt `
  --test-ids baseline\tests\fixtures\test_ids.txt `
  --split-report baseline\tests\fixtures\synthetic_split_report.json `
  --m1-gate data\reports\m1\m1_gate_report.json `
  --output $formalOutputPath 2>&1
$formalExitCode = $LASTEXITCODE
if ($formalExitCode -ne 2 -or `
    (Test-Path -LiteralPath $formalOutputPath) -or `
    (($formalOutput -join "`n") -notmatch 'M1 gate has not passed')) {
  throw 'formal current-gate rejection did not fail closed as expected'
}
if (Test-Path -LiteralPath $formalOutputPath) {
  Remove-Item -LiteralPath $formalOutputPath -Force
}
```

预期：退出码为 `2`，合并输出中包含 `baseline blocked: M1 gate has not passed` 门禁消息，且不产生报告。当前正式Gold=0、M1未通过，因而没有正式训练/test指标、CreatorShift增益或M4通过结论。

P4正式实验协议已冻结在`docs/superpowers/specs/2026-08-08-p4-experiment-protocol-design.md`，但本节命令仍只验证四方法合成工程链和正式门禁。当前报告没有CreatorShift第五行、creator-cluster bootstrap区间或一次性test运行注册表；因此即使上述测试全绿，也只能报告点估计工程能力，不能声称稳定增益、统计显著性或M4通过。
...
兼容路由 `/analyze` 使用同一服务，但新代码应优先使用 `/api/v1/analyze`。`/analyze` 是程序接口；交互式证据画布、创作者时间线和法规工作台已在同源 `/workbench` 提供。
...
- 先确保第3节`405 passed, 2 skipped, 1 warning`的本地基线已经保存。
...
- FastAPI已接统一单条/批量分析、URL预览/确认、报告和run查询；同源无构建`/workbench`已提供交互式开发者研究工作台，但不替代团队UAT；
- 真实 LLM/LangSmith 是外部能力接入，不是论文算法创新；
- A2A、真实平台URL适配、完整法规/平台规则库和学习型CreatorShift模型尚未完成；P5.1通用URL服务契约、P5.2同源工作台、P1 Adapter、MCP Gateway、Knowledge MCP、小规模官方法规基线和确定性CreatorShift图节点已接入；
...
当前P1 Adapter、Evidence Adapter、LangGraph主图、确定性CreatorShift节点、保守Judge、Judge后混合法规检索、报告与run持久化已经接成默认本地主链。P4已有版本化mean/max/EMA夹具、bootstrap和risk-coverage工程报告；P5.1已有批量分析和安全URL预览/确认服务边界，P5.2已有同源无构建工作台、完整run视图和导出，P5.7已有离线安全工程验收。仍未完成的是M1正式Gold与治理证据、学习型CreatorShift/真实校准、完整法规覆盖、跨机器MCP、真实A2A、真实小红书/B站适配、团队UAT和P5.3～P5.6。MCP、A2A、RAG、LLM和网页都是证据链的能力或交互载体；CreatorShift才是需要通过真实数据、严格时间切分和强基线证明的研究主张。测试库的价值，是让工程能力的“已完成”、研究假设的“待验证”和产品路线的“尚未实现”始终保持清楚。
