...
> 当前实测基线：2026-07-26，分支 `P2_Tool-Compartment-Model-Tooling`
...
| LangGraph 起步 Agent 图 | 已有规则/LLM 起步实现，但尚未接入新的完整证据主链 | 默认 Agent 测试；零 Key 演示 |
...
Set-Location -LiteralPath 'D:\AAA Jobs\Detection-of-Recessive-Advertising\implicit-ad-agent'
python -m compileall -q impad tests app.py
```
...
- 不能证明真实平台 URL 可以采集；
- 不能证明 P1 Schema 已经适配成运行时 `PostRecord/CaptureStatus`，合并代码不等于完成主链集成；
...
其中 `tests\data` 为 29 个，原有契约、编排、MCP、RAG 与 CreatorShift 独立模块为 80 个。这组命令适合快速检查 P1 数据层和新基础模块，但不能代替第 3 节全量回归。
...
- 先确保第 3 节 `171 passed, 2 skipped` 的本地基线已经保存。
...
`app.py` 导入的是 `impad.graph`，所以 `/analyze` 不属于默认零 Key 验收。
...
- A2A、网页工作台、P1 Adapter、MCP Gateway 和真实法规库尚未完成；P1 已合并不改变这些运行时集成状态；
...
P1 content_record / 手工 RawRequest
  → （待完成 P1 Adapter）PostRecord + CaptureStatus
...
当前这些部件并没有全部接成一条正式主链：P1 Adapter、Evidence Adapter、主图集成、Judge 新契约输出和 Judge 后法规报告仍是后续工作。MCP、A2A、RAG、LLM 和网页都是证据链的能力或交互载体；CreatorShift 才是需要通过真实数据、严格时间切分和强基线证明的研究主张。测试库的价值，是让工程能力的“已完成”、研究假设的“待验证”和产品路线的“尚未实现”始终保持清楚。
