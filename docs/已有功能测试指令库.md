...
| 独立合并历史分类基线 | `baseline/`四方法固定Logistic Regression、M1/split fail-closed、共同cohort、版本/哈希和隐私安全聚合报告；最新专项`44 passed` | 第4.17节；仅合成fixture工程验证，正式Gold=0且M1未通过 |
...
以下命令均从仓库根目录运行，并显式使用项目虚拟环境解释器；只读合成夹具，不写入真实私有数据路径。依赖安装可能访问包索引，其余命令为零Key、零联网工程检查。
...
最新实测：baseline专项 `44 passed`，`compileall`退出码为0。该结果只证明独立基线工程行为，不是M1或M4验收。
...
预期：退出码为 `2`，stderr含 `baseline blocked: M1 gate has not passed`，且不产生报告。当前正式Gold=0、M1未通过，因而没有正式训练/test指标、CreatorShift增益或M4通过结论。
