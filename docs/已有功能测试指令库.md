...
> 当前实测基线：2026-07-27，分支 `P2_Tool-Compartment-Model-Tooling`
...
| Detection MCP 与 Gateway | 七工具 stdio 协议、主图 MCP 模式和失败本地回落已实现 | `tests\protocols\mcp`；`tests\orchestration\test_mcp_gateway.py` |
| 法规 RAG 与 Knowledge MCP | 小规模官方语料、Chroma/hash 检索、引用守卫、Knowledge MCP 和 Judge 后报告已实现 | `tests\rag`；Knowledge MCP 测试；service 测试 |
| CreatorShift 基础 | 防未来泄漏的历史视图、三种池化和偏移解释 | `tests\creator_shift` |
| FastAPI 与统一服务 | `/api/v1/analyze`、能力、run 查询和兼容 `/analyze` 共用 AnalysisService | `tests\api`；`tests\services` |
...
- 真实数据上的三分类精度、校准、CreatorShift 增益和论文结论；
- A2A 专家间任务交换；
...
- 三分类、暗广排序/校准与覆盖率指标可以离线计算。

它不证明远程 MCP 服务可达、完整法规覆盖、真实分类精度或 M1 已通过。

### 4.10 一次运行 P1 数据层与本轮独立模块
...
> `/health`、`/api/v1/capabilities`、`/api/v1/analyze` 和 `/api/v1/runs/{run_id}` 均由统一 `AnalysisService` 支持，可以纳入零 Key 本地工程检查。
...
- 先确保第 3 节 `276 passed, 2 skipped` 的本地基线已经保存。
...
- FastAPI 已接统一分析、报告和run查询，但还不是交互式研究工作台；
- 真实 LLM/LangSmith 是外部能力接入，不是论文算法创新；
- A2A、网页工作台、平台URL、完整法规/平台规则库和真实CreatorShift模型尚未完成；P1 Adapter、MCP Gateway、Knowledge MCP和小规模官方法规基线已接入；
