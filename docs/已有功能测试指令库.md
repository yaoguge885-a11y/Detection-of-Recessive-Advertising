...
| 独立合并历史分类基线 | `baseline/`四方法固定Logistic Regression、M1/split fail-closed、共同cohort、版本/哈希和隐私安全聚合报告；最新专项`48 passed` | 第4.17节；仅合成fixture工程验证，正式Gold=0且M1未通过 |
...
最新实测：baseline专项 `48 passed`（contracts17/features14/runner12/CLI5），`compileall`退出码为0。该结果只证明独立基线工程行为，不是M1或M4验收。
