...
- `train/build_balanced_dataset.py`：LLM 辅助构建三类平衡数据集
- `train/augment_hidden_ad.py`：中文数据增强
- `train/test_training_methods_v2.py`：超参网格实验
- `train/evaluate_model.py`：P/R/F1/混淆矩阵评估
- `inference_service.py`：训练后模型的推理封装
- `benchmarks/`：桌面版已跑出的 benchmark JSON
- `requirements.txt`：独立的 PaddlePaddle 依赖

建议使用独立虚拟环境，避免与主项目依赖冲突：
...
1. 确定工程性论文题目。
2. 研究型论文在 8 月 16 日前准备纲要、关键词、目录和综述，并明确人员分工；重点提出假设、证明观点并包含深度学习、模型评估与论证。
3. 查阅 MMM 2024—2026 历年论文，优先寻找与大创相关的工作。
4. 若研究型论文方向不可行，再从 FlowUs 备选论题中选择。
5. 使用 EndNote 导入并管理论文 PDF 原文。
