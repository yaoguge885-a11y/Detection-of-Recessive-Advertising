...
**关键参数（v1.1 补充）：**
- `"think": false`（**顶层**参数）：禁用 Qwen3.5 默认 thinking，避免长推理截断结构化 JSON；放进 `options` 里不生效。
- `"keep_alive": "30m"`：模型常驻，减少重复冷启动；实际延迟和显存占用依目标机器实测。
- 服务器管理工具 `data-tooling/annotation/ollama_server.py`：显式启动、状态查询和模型预热，并报告实际发现的 `OLLAMA_MODELS` 目录。
